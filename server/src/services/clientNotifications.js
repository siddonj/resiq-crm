 ...
 */
async function sendTicketAssignedNotification(employeeEmail, employeeName, clientName, ticketSubject, ticketId) {
  try {
    const ticketLink = `${process.env.API_URL || 'http://localhost:5000'}/api/tickets/${ticketId}`
      ...
      <html>
        <head>
          <style>
            ...
            .header { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 30px; border-radius: 8px 8px 0 0; text-align: center; }
            .content { background: #f9f9f9; padding: 30px; border-radius: 0 0 8px 8px; }
            .ticket-box { background: white; padding: 15px; border-radius: 5px; border-left: 4px solid #667eea; margin: 20px 0; }
            .button { display: inline-block; background: #667eea; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; margin: 20px 0; font-weight: bold; }
            .footer { text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee; font-size: 12px; color: #666; }
          </style>
        </head>
        <body>
          <div class="container">
            <div class="header">
              <h1 style="margin: 0;">🎟️ New Support Ticket</h1>
              <p style="margin: 10px 0 0 0;">You've been assigned a ticket</p>
            </div>
            <div class="content">
              <p>Hi ${employeeName},</p>
              ...
              <div class="ticket-box">
                ...
                <strong>From Client:</strong> ${clientName}<br>
              ...
              <p>Please review the ticket and respond promptly. You can view the full details and add replies in the Help Desk.</p>
            </div>
            <div class="footer">
              <p>© 2026 ResiQ CRM. All rights reserved.</p>
            </div>
    ...
    console.log(`✓ Ticket assigned notification sent to ${employeeEmail}`)
    return true
  } catch (err) {
    console.error(`Failed to send ticket notification to ${employeeEmail}:`, err.message)
    return false
  }
}

/**
 * Send ticket reply notification to assigned employee
 */
async function sendTicketReplyNotification(employeeEmail, employeeName, clientName, ticketSubject, ticketId) {
  try {
    const ticketLink = `${process.env.API_URL || 'http://localhost:5000'}/api/tickets/${ticketId}`
      ...
      <html>
        <head>
          <style>
            ...
            .header { background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white; padding: 30px; border-radius: 8px 8px 0 0; text-align: center; }
            .content { background: #f9f9f9; padding: 30px; border-radius: 0 0 8px 8px; }
            .ticket-box { background: white; padding: 15px; border-radius: 5px; border-left: 4px solid #f5576c; margin: 20px 0; }
            .button { display: inline-block; background: #f5576c; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; margin: 20px 0; font-weight: bold; }
            .footer { text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #eee; font-size: 12px; color: #666; }
          </style>
        </head>
        <body>
          <div class="container">
            <div class="header">
              ...
              <p style="margin: 10px 0 0 0;">Client has replied to your ticket</p>
            </div>
            <div class="content">
              <p>Hi ${employeeName},</p>
              <p>${clientName} has replied to your ticket.</p>
              ...
              <p style="text-align: center;">
                <a href="${ticketLink}" class="button">View Reply</a>
              </p>
              
              <p>Please respond promptly to keep the conversation going.</p>
            </div>
            <div class="footer">
              <p>© 2026 ResiQ CRM. All rights reserved.</p>
            </div>
    ...
    console.log(`✓ Ticket reply notification sent to ${employeeEmail}`)
    return true
  } catch (err) {
    console.error(`Failed to send ticket reply email to ${employeeEmail}:`, err.message)
    return false
  }
}
